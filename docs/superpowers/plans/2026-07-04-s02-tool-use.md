...
全部 6 个任务完成，27/27 测试通过，已推送 origin/main。验收：s02 实时跑通（模型对"列出 .py 文件"自动选 `glob` 工具，返回 12 个文件并总结）。

| 任务 | commit | 说明 |
|---|---|---|
| 1 包骨架 | `3f7f1a8` | 初始化包 |
| 2 tools | `0a021f2` | 5 工具 + safe_path + run_tool 分发器（19 测试） |
| 3 agent | `8de6196` | agent_loop 按名分发 + 多工具（4 测试） |
| 4 config | `c015b16` | 5 工具定义 + 系统提示（4 测试） |
| 5 cli + __main__ | `7632297` | REPL 入口（导入冒烟通过） |
| 6 README + 全测 + push | `0a7acb5` | 27 passed，推送 |
