...
全部 8 个任务完成，s05 54/54 测试通过（全量 s01-s05 163 passed），已推送 origin/main。验收：s05 实时跑通——模型首工具即 `todo_write` 列 5 步，执行中 status `pending→in_progress→completed`，nag 未触发，demo_pkg 测试通过。

| 任务 | commit | 说明 |
|---|---|---|
| 1 包骨架 | `3615cbb` | 初始化包 |
| 2 tools | `0506bdc` | s04 工具 + todo_write（27 测试） |
| 3 hooks | `78226a5` | s04 原样复制（12 测试） |
| 4 todo | `91b94cc` | TodoNag（6 测试） |
| 5 agent | `4e42c6b` | agent_loop 注入 nag（6 测试：4 s04 + 2 nag） |
| 6 config | `424cb8f` | 6 工具 + 规划提示（3 测试） |
| 7 cli + __main__ | `cfde09e` | REPL 入口 |
| 8 README + 全测 + 冒烟 + push | `9cc9eac` + 后续 docs | 163 passed，冒烟通过，推送 |
