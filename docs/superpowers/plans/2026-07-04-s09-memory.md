...
全部 7 个任务完成，s09 137/137 测试通过（全量 s01-s09 572 passed），已推送 origin/main。验收：s09 实时跑通——`[Memory: extracted 3 new memories]`，`.memory/` 落 3 记忆文件 + MEMORY.md 索引。

| 任务 | commit | 说明 |
|---|---|---|
| 1+2 骨架+memory | `49eb4f3` | memory.py Memory 类（17 测试） |
| 3 s08 复制 | `8257e5b` | tools/skills/hooks/todo/subagent/compact（99 测试） |
| 4 agent | `977a85d` | memory 集成（注入+提取+整合）（16 测试） |
| 5+6 config+cli | `f273713` | config（同 s08）+ REPL 接线 Memory |
| 7 README + 全测 + 冒烟 + push | 后续 docs + fix | 572 passed；冒烟通过；max_tokens fix（`b7b25c0`） |

**关键发现**：glm-5.2 是推理模型，memory 的 LLM 调用需大 `max_tokens`（select 2000/extract 4000/consolidate 4000）——参考的 200/800/3000 会被 thinking 吃光、text 块为空导致提取无结果。fix commit `b7b25c0`。
