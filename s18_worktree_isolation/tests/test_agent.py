...
from s18_worktree_isolation.system_prompt import reset_cache
...
def _reset():
    from s18_worktree_isolation import background, cron
    reset_cache()
...
def test_with_retry_429_in_loop(monkeypatch):
    from s18_worktree_isolation import recovery
...
def test_background_notification_injected_after_results():
    from s18_worktree_isolation import background
...
def test_cron_queue_injected_as_scheduled():
    from s18_worktree_isolation import cron
    from s18_worktree_isolation.cron import CronJob
