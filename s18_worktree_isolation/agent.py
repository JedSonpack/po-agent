"""核心 agent 循环（s18）：s17 + worktree（agent_loop 不变；隔离在 Team wt_ctx）。"""
from typing import Callable

from s18_worktree_isolation.system_prompt import build_context, get_system_prompt
from s18_worktree_isolation.recovery import (RecoveryState, with_retry, is_prompt_too_long_error,
                                           DEFAULT_MAX_TOKENS, ESCALATED_MAX_TOKENS,
                                           MAX_RECOVERY_RETRIES, CONTINUATION_PROMPT)
from s18_worktree_isolation.background import (should_run_background, start_background_task,
                                             collect_background_results)
from s18_worktree_isolation.cron import consume_cron_queue
