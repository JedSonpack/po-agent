import subprocess
import pytest
import s20_comprehensive.tools as tools_mod
from s20_comprehensive.tools import (
...
def test_timeout(monkeypatch):
    ...
    monkeypatch.setattr("s20_comprehensive.tools.subprocess.run", fake_run)
...
def test_output_truncation(monkeypatch):
    class FakeResult:
        stdout = "x" * 60000
        stderr = ""
    monkeypatch.setattr("s20_comprehensive.tools.subprocess.run",
                        lambda *a, **k: FakeResult())
    assert len(run_bash("echo big")) == 50000


def test_safe_path_within_workspace(monkeypatch, tmp_path):
    monkeypatch.setattr("s20_comprehensive.tools.WORKDIR", tmp_path)
...
def test_safe_path_with_cwd(tmp_path):
    from s20_comprehensive.tools import safe_path
    (tmp_path / "sub").mkdir()
...
def test_run_bash_with_cwd(tmp_path):
    from s20_comprehensive.tools import run_bash
    out = run_bash("pwd", cwd=tmp_path)
    assert str(tmp_path) in out


def test_run_write_read_with_cwd(tmp_path):
    from s20_comprehensive.tools import run_write, run_read
