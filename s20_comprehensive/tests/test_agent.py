...
from s20_comprehensive.system_prompt import reset_cache
...
def _reset():
    from s20_comprehensive import background, cron
    reset_cache()
...
def test_with_retry_429_in_loop(monkeypatch):
    from s20_comprehensive import recovery
...
def test_background_notification_injected_after_results():
    from s20_comprehensive import background
...
def test_cron_queue_injected_as_scheduled():
    from s20_comprehensive import cron
    from s20_comprehensive.cron import CronJob
...
def test_tool_pool_refreshes_each_turn():
    ...
    from s20_comprehensive.mcp import ToolPool, mcp_clients

    pool = ToolPool(
        [{"name": "bash", "input_schema": {}},
         {"name": "connect_mcp", "input_schema": {}}],
        {"bash": lambda command: "OUT",
         "connect_mcp": lambda name: __import__("s20_comprehensive.mcp", fromlist=["connect_mcp"]).connect_mcp(name)})
