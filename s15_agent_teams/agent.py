"""核心 agent 循环（s15）：s14 + 团队（agent_loop 不变；团队工具经 run_tool 分发，inbox 注入在 cli 层）。"""
from typing import Callable

from s15_agent_teams.system_prompt import build_context, get_system_prompt
from s15_agent_teams.recovery import (RecoveryState, with_retry, is_prompt_too_long_error,
                                           DEFAULT_MAX_TOKENS, ESCALATED_MAX_TOKENS,
                                           MAX_RECOVERY_RETRIES, CONTINUATION_PROMPT)
from s15_agent_teams.background import (should_run_background, start_background_task,
                                             collect_background_results)
from s15_agent_teams.cron import consume_cron_queue
