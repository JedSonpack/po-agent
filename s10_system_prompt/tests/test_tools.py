import subprocess
import pytest
import s10_system_prompt.tools as tools_mod
from s10_system_prompt.tools import (
...
def test_timeout(monkeypatch):
    ...
    monkeypatch.setattr("s10_system_prompt.tools.subprocess.run", fake_run)
...
def test_output_truncation(monkeypatch):
    class FakeResult:
        stdout = "x" * 60000
        stderr = ""
    monkeypatch.setattr("s10_system_prompt.tools.subprocess.run",
                        lambda *a, **k: FakeResult())
    assert len(run_bash("echo big")) == 50000


def test_safe_path_within_workspace(monkeypatch, tmp_path):
    monkeypatch.setattr("s10_system_prompt.tools.WORKDIR", tmp_path)
