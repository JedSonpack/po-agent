...
def test_memory_index_appended_to_system():
    ...
    spy = SpyMemory()
    spy.section = "\n\nMemories available:\n- [X](x.md) — dx"
    agent_loop(client=CaptureClient(), model="m", system="BASE", tools=[], messages=[{"role": "user", "content": "x"}],
               run_tool=lambda n, i: "OUT", trigger=lambda ev, *a: None, memory=spy)
...
def test_memory_injected_into_user_turn():
    ...
    agent_loop(client=CaptureClient(), model="m", system="s", tools=[], messages=[{"role": "user", "content": "hello"}],
               run_tool=lambda n, i: "OUT", trigger=lambda ev, *a: None, memory=spy)
