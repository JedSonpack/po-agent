import subprocess
import pytest
import s09_memory.tools as tools_mod
from s09_memory.tools import (
...
def test_timeout(monkeypatch):
    ...
    monkeypatch.setattr("s09_memory.tools.subprocess.run", fake_run)
...
def test_output_truncation(monkeypatch):
    class FakeResult:
        stdout = "x" * 60000
        stderr = ""
    monkeypatch.setattr("s09_memory.tools.subprocess.run",
                        lambda *a, **k: FakeResult())
    assert len(run_bash("echo big")) == 50000


def test_safe_path_within_workspace(monkeypatch, tmp_path):
    monkeypatch.setattr("s09_memory.tools.WORKDIR", tmp_path)
