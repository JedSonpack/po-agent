import subprocess
import pytest
from s03_permission.tools import (
    run_bash, safe_path, run_read, run_write, run_edit, run_glob, run_tool,
)


# ---- run_bash（s03 简化版，无 DANGEROUS——危险拦截移到权限层）----
...
def test_timeout(monkeypatch):
    ...
    monkeypatch.setattr("s03_permission.tools.subprocess.run", fake_run)
...
def test_output_truncation(monkeypatch):
    class FakeResult:
        stdout = "x" * 60000
        stderr = ""
    monkeypatch.setattr("s03_permission.tools.subprocess.run",
                        lambda *a, **k: FakeResult())
    assert len(run_bash("echo big")) == 50000


# ---- safe_path / file tools（同 s02）----
def test_safe_path_within_workspace(monkeypatch, tmp_path):
    monkeypatch.setattr("s03_permission.tools.WORKDIR", tmp_path)
