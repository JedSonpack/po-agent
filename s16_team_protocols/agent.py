"""核心 agent 循环（s16）：s15 + 协议（agent_loop 不变；协议工具经 run_tool 分发）。"""
from typing import Callable

from s16_team_protocols.system_prompt import build_context, get_system_prompt
from s16_team_protocols.recovery import (RecoveryState, with_retry, is_prompt_too_long_error,
                                           DEFAULT_MAX_TOKENS, ESCALATED_MAX_TOKENS,
                                           MAX_RECOVERY_RETRIES, CONTINUATION_PROMPT)
from s16_team_protocols.background import (should_run_background, start_background_task,
                                             collect_background_results)
from s16_team_protocols.cron import consume_cron_queue
